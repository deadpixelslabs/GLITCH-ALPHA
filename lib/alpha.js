...
async function fetchJson(url,timeout=9000,headers={}){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"application/json","user-agent":"dead-pixels-glitch-alpha/3.7.2",...headers},signal:ctrl.signal});
...
function scorePool(p){
  ...
  score=clamp(score);
  return {score,components:{momentum:Math.round(momentum),liquidity:Math.round(liquidity),activity:Math.round(activity),flow:Math.round(flow),novelty:Math.round(maturity),structure:Math.round(structure)},verdict:score>=82?"ALPHA":score>=70?"HOT":score>=55?"ACTIVE":score>=40?"WATCH":"COLD",model:"V3.7.2"};
}
...
async function getBlockscoutDetail(address){
  const a=lower(address);const base=[
    `${BLOCKSCOUT}/api/v2/addresses/${a}`,
    `${BLOCKSCOUT}/api/v2/tokens/${a}`,
    `${BLOCKSCOUT}/api/v2/tokens/${a}/counters`
  ];
  const s=await Promise.allSettled(base.map(u=>cachedJson(u,30000,9000)));
  const out={address:s[0].status==="fulfilled"?s[0].value:null,token:s[1].status==="fulfilled"?s[1].value:null,counters:s[2].status==="fulfilled"?s[2].value:null,holders:null,source:null,search:null};
  const extra=await Promise.allSettled([
    cachedJson(`${BLOCKSCOUT}/api/v2/tokens/${a}/holders`,20000,9000),
    cachedJson(`${BLOCKSCOUT}/api/v2/smart-contracts/${a}`,60000,9000)
  ]);
  out.holders=extra[0].status==="fulfilled"?extra[0].value:null;out.source=extra[1].status==="fulfilled"?extra[1].value:null;
  const t=out.token||out.address?.token||{};const known=[out.counters?.token_holders_count,t?.holders_count,t?.holders,out.address?.token?.holders_count,out.address?.token?.holders].map(x=>n(x)).filter(x=>x>0);
  // Some fresh Robinhood Blockscout records briefly report "0" in /counters while token search already has a real count.
  if(!known.length){try{out.search=await cachedJson(`${BLOCKSCOUT}/api/v2/search?q=${encodeURIComponent(a)}`,30000,9000);}catch{}}
  return out;
}
...
function holderStats(holderBody,totalSupplyRaw,creator,pairAddresses=[]){
  const total=safeBig(totalSupplyRaw);const excluded=new Set(["0x0000000000000000000000000000000000000000","0x000000000000000000000000000000000000dead",...pairAddresses.map(lower)]);const raw=(holderBody?.items||[]).map(x=>({address:extractAddress(x.address_hash||x.address),valueRaw:String(x.value||"0")})).filter(x=>addr(x.address));
  ...
  return {top10Pct:pctBig(top10,total),top20Pct:pctBig(top20,total),creatorPct:creatorItem?pctBig(safeBig(creatorItem.valueRaw),total):null,top:raw.slice(0,20).map(x=>({...x,pct:pctBig(safeBig(x.valueRaw),total)})),sampleCount:raw.length,sampleHasMore:!!holderBody?.next_page_params};
}
...
async function detail(address,pairAddress=null,sourceHint=null){
  ...
  const counts=[bs?.counters?.token_holders_count,bt?.holders_count,bt?.holders,bs?.address?.token?.holders_count,bs?.address?.token?.holders,searchHolderCount(bs?.search,a)].map(x=>n(x)).filter(x=>x>0);
  const holdersCount=counts.length?Math.max(...counts):null;const holders={count:holdersCount,countLabel:holdersCount?String(holdersCount):(hs.sampleCount?`${hs.sampleCount}${hs.sampleHasMore?"+":""}`:null),countSource:holdersCount?"BLOCKSCOUT INDEX":(hs.sampleCount?"BLOCKSCOUT HOLDER SAMPLE":null),...hs};
  ...
  const gl=market?scorePool(market):{score:0,verdict:"UNPRICED",components:{momentum:0,liquidity:0,activity:0,flow:0,novelty:0,structure:0},model:"V3.7.2"};gl.scoreScope="MARKET_PAIR";
  ...
  return {chainId:CHAIN_ID,address:a,official:officialToken(a),projectRole:officialToken(a)?"DEAD PIXELS ECOSYSTEM TOKEN":null,meta:{...meta,imageUrl:market?.imageUrl||bt?.icon_url||null},market,aggregateMarket,pools:pools.slice(0,20),holders,security:sec,risk,glitch:gl,confidence,deployer:{address:creator,creationTx:bs?.address?.creation_transaction_hash||null},trades,whales,whaleThreshold,consensus,pairSelection:{requested:poolId(wantedPair)?wantedPair:null,selected:market?.pairAddress||null,source:market?.source||null},sources:{DEXSCREENER:dexS.status==="fulfilled"?"ONLINE":"ERROR",GECKOTERMINAL:gtS.status==="fulfilled"?"ONLINE":"ERROR",BLOCKSCOUT:bsS.status==="fulfilled"?"ONLINE":"ERROR",RPC:metaS.status==="fulfilled"?"ONLINE":"ERROR"},generatedAt:new Date().toISOString()};
}
...
module.exports={overview,catalog,search,justBorn,detail,ohlcv,status,recentMintSignals,scorePool,CHAIN_ID,NETWORK,GLITCH,_test:{chooseSubject,normalizeDex,normalizeGT,holderStats,searchHolderCount,bestMarketsByToken,poolId}};
